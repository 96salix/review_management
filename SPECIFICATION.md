...
### 5.9. 画面コンポーネントとルーティング

アプリケーションの各画面は、以下のReactコンポーネントによって実装されており、指定されたURLパスにマッピングされています。

| URLパス | コンポーネントファイル | 説明 |
| :--- | :--- | :--- |
| `/` | `ReviewList.tsx` | 全てのレビュー依頼を一覧表示するメインページ。 |
| `/my-reviews` | `MyReviews.tsx` | 現在のユーザーがレビュアーとして割り当てられているレビュー依頼を一覧表示する。 |
| `/new` | `NewReview.tsx` | 新しいレビュー依頼を作成するためのフォーム画面。 |
| `/reviews/:id` | `ReviewDetail.tsx` | 特定のレビュー依頼の詳細を表示する。ステージやコメントの確認・操作を行う。 |
| `/reviews/:id/edit` | `EditReview.tsx` | 既存のレビュー依頼を編集するためのフォーム画面。 |
| `/users` | `UserManagement.tsx` | 全てのユーザーを管理（作成・編集・削除）する画面。 |
| `/stage-templates` | `StageTemplateManagement.tsx` | レビュー依頼の雛形となるステージテンプレートを管理する画面。 |
